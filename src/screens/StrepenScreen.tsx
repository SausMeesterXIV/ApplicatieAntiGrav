    ...
    <div className="flex flex-col min-h-screen bg-gray-50 dark:bg-[#0f172a] transition-colors duration-200">
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-6 pb-24">
        ...
        <section>
          ...
          <div className="bg-white dark:bg-[#1e2330] p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 transition-colors">
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">Kies welk drankje je direct wilt kunnen strepen vanaf het startscherm.</p>
            <div className="grid grid-cols-2 gap-2">
              {drinks.filter(d => d.name !== SPECIAL_DRINKS.BAK_FREEDOM).map(drink => (
                <button key={`quick-${drink.id}`} onClick={() => onUpdateUser({ ...currentUser, id: currentUser?.id || '', quickDrinkId: String(drink.id) } as User)} className={`px-3 py-2.5 text-xs font-bold rounded-xl border transition-all flex items-center justify-between ${String(currentUser?.quickDrinkId || (drinks.length > 0 ? drinks[0].id : null)) === String(drink.id) ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 text-blue-700' : 'bg-gray-50 dark:bg-gray-800/50 border-gray-100 text-gray-600'}`}>
