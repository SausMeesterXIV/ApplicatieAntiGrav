            ...
            return (
              <div key={leader.userId} className={`flex items-center justify-between p-3.5 rounded-2xl transition-colors border ${isGold ? 'bg-gradient-to-r from-yellow-50 to-amber-50 dark:from-yellow-900/10 dark:to-amber-900/10 border-yellow-300 dark:border-yellow-700/50 shadow-md' : 'bg-white dark:bg-[#1e293b] border-gray-100 dark:border-gray-800 shadow-sm'}`}>
                <div className="flex items-center gap-4 min-w-0">
                  ...
                  <UserAvatar user={getUser(leader.userId)} size="lg" className={isGold ? 'border-[3px] border-yellow-400 shadow-lg' : isDuo ? 'border-[3px] border-purple-500 shadow-md' : ''} />
                ...
                <div className="flex items-end justify-center gap-3 pt-4 pb-2">
                    ...
                    return (
                      <div className="flex flex-col items-center w-24">
                        <div className="relative mb-2">
                          <UserAvatar user={getUser(leader.userId)} size="lg" className="border-3 border-gray-300 dark:border-gray-500 shadow-lg bg-gray-200" />
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="flex flex-col items-center w-28 -mt-4">
                        <div className="relative mb-2">
                          <UserAvatar user={getUser(leader.userId)} size="xl" className="border-4 border-yellow-400 shadow-xl shadow-yellow-400/30 bg-yellow-100 ring-4 ring-yellow-400/20" />
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="flex flex-col items-center w-24">
                        <div className="relative mb-2">
                          <UserAvatar user={getUser(leader.userId)} size="lg" className="border-3 border-orange-400 dark:border-orange-500 shadow-lg bg-orange-100" />
                          <span className="absolute -top-1 -right-1 text-lg">🥉</span>
                        </div>
                  ...
                  return (
                    <div key={leader.userId} className={`flex items-center justify-between p-4 ${index !== 2 ? 'border-b border-gray-100 dark:border-gray-800' : ''} ${isGold ? 'bg-yellow-50/50 dark:bg-yellow-900/5' : ''}`}>
                      <div className="flex items-center gap-3">
                        ...
                        <UserAvatar user={getUser(leader.userId)} size="md" className={isGold ? 'border-[2px] border-yellow-400' : isDuo ? 'border-[2px] border-purple-500' : ''} />
                        <div>
                  ...
                  <div className="relative z-10 flex justify-between items-center">
                    ...
                    <div className="flex -space-x-3 shrink-0">
                      {duoBierpongWinners.slice(0, 2).map((id, index) => (
                        <UserAvatar user={getUser(id)} size="lg" className="border-2 border-white dark:border-[#1e2330] shadow-sm relative overflow-hidden" />
                      ))}
                    </div>
                  </div>
                </div>
                  ...
                  {[...bierpongGames].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()).slice(0, 10).map((game, i) => {
                    ...
                    return (
                      <div key={game.id || i} className="p-3 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <UserAvatar user={getUser(winner)} size="sm" className="border border-green-500 bg-green-50" />
                          <div className="text-xs font-black text-gray-400">VS</div>
                          <UserAvatar user={getUser(loser)} size="sm" className="border border-red-300 bg-red-50 opacity-80 grayscale" />
                        </div>
        ...
        const renderPlayerChip = (pid: string, removable: boolean, onRemove?: () => void) => {
          ...
          return (
            <div key={pid} className="flex items-center gap-2 py-1.5 px-2.5 rounded-lg bg-gray-100 dark:bg-gray-800">
              <UserAvatar user={u} size="sm" className="border border-gray-200 dark:border-gray-600" />
                        ...
                        <div className="space-y-1 max-h-40 overflow-y-auto">
                          {filteredUsers.map(u => (
                            <button key={u.id} onClick={() => { setSelectedPlayers(prev => [...prev, u.id]); setPlayerSearch(''); }}
                              ...
                              <UserAvatar user={u} size="sm" className="border border-gray-200 dark:border-gray-700" />
