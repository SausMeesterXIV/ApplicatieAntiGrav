...
glob('src/screens/**/*.tsx', (err, files) => {
  ...
  files.forEach(file => {
    ...
    if (content.indexOf('useOutletContext') !== -1 && content.indexOf('AppContextType') !== -1) {
      ...
      const regex = /const\s+{\s*([^}]+)\s*}\s*=\s*useOutletContext<(?:AppContextType|any)>\(\);/;
      ...
      if (match) {
        ...
        vars.forEach(v => {
          ...
          // special case for fries tracking
